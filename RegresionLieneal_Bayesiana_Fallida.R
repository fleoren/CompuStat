...
library(ggplot2)
prior.beta0 <- function(x) dnorm(x, 0, .2)
prior.beta1 <- function(x) dnorm(x, 0, .2)
prior.beta2 <- function(x) dnorm(x, 0, .2)
prior.beta3 <- function(x) dnorm(x, 0, .2)
prior.sigma <- function(x) dgamma(x, 5, 40)
...
data <- matrix(cbind(iris$Sepal.Width,iris$Petal.Length,iris$Petal.Width),ncol=3)
ys <- matrix(iris$Sepal.Length, ncol=1)
...
# 1)
cppFunction('
            double objdens(NumericMatrix data, NumericVector ys, NumericVector theta){
            double lkh, logprior;
            int m=data.nrow();
            NumericMatrix X(m,3);
            NumericVector Y(m);
            ...
            lkh=0;
            for (int i=0; i<m; i++){
            lkh += - (1/(2*pow(theta[4],2))) * pow((Y[i]- 1*theta[0] - X(i,1)*theta[1] - X(i,2)*theta[2] - X(1,3)*theta[3] ),2) -log(theta[4]) ;
            }
            // Compute logprior
            logprior = R::dnorm(theta[0], 0,.5, true) + R::dnorm(theta[1], 0,.5, true) + R::dnorm(theta[2], 0,.5, true) + R::dnorm(theta[3], 0,.5, true) +  R::dgamma(theta[4], 5.0, 1.0/40.0, true);
            // Log of target density
            return lkh + logprior;
            }')
objdens(data,ys, c(0,0,0,0,5))
